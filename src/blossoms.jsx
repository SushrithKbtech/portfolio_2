...
function GLBBlossoms() {
  ...
  return <Field geometry={geometry} />
}

// Five petals and a centre. Crude next to the real mesh, but a petal silhouette all the same.
function ProcBlossoms() {
  const geometry = useMemo(() => proceduralBlossom(), [])
  return <Field geometry={geometry} />
}

export default function Blossoms() {
  return (
    <Optional name="/models/blossom.glb" fallback={<ProcBlossoms />}>
      <GLBBlossoms />
    </Optional>
  )
}
